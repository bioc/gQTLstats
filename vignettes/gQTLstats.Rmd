...
filtFDR = storeToFDR(prst, 
   xprobs=c(seq(.05,.95,.05),.975,.990,.995,.9975,.999, 
   .9995, .9999, .99999), filter = function(x)
...
by gene, for observed and permuted measures.  These functions
can be slow, as they involve mapping the original set of
jobs through a best-SNP-per-probe selection process for the observed
and permuted scores.  Concurrent computing capabilities of
...
We can use the `FDRsupp` instances with `ciseStore`
to list the SNP-probe pairs meeting specific FDR conditions.

```{r doenums,cache=TRUE}
rawEnum = enumerateByFDR(prst, rawFDR, threshold=.05) 
GenomicRanges::unlist(GRangesList(rawEnum))
filtEnum = enumerateByFDR(prst, filtFDR, threshold=.05) 
GenomicRanges::unlist(GRangesList(filtEnum))
```
